...
bool Snapshot::read(const QString & filename)
{
	...
	//Added links as a parameter in physical and virtual elements
	Requests requests = getRequests();
	Elements virtualNodes = Elements();
	double sumVirtualLinks = 0.0;
	for (Requests::iterator i = requests.begin(); i != requests.end(); i++) {
		Request * request = *i;
		Elements nodes = request->getNodes();
		virtualNodes.insert(nodes.begin(), nodes.end());
		sumVirtualLinks += request->getAllLinksParameters();
	}

	int maxSumLinks = 0;

	Parameter * param = new Parameter(std::string("links"));
	ElementFactory::parameters[QString("links")] = param;

	for (Elements::iterator j = virtualNodes.begin(); j != virtualNodes.end(); j++) {
		Element * elem = *j;
		int sumLinks = elem->toNode()->sumAdjacentLinks();
		if ( sumLinks > maxSumLinks)
			maxSumLinks = sumLinks;

		ParameterValue * pv = new ParameterInt(sumLinks);
		elem->parameters[param] = pv;
	}

	Elements physicalNodes = getNetwork()->getNodes();
	for (Elements::iterator k = physicalNodes.begin(); k != physicalNodes.end(); k++) {
		Element * node = *k;
		int sumLinks = node->toNode()->sumAdjacentLinks();
		ParameterValue * pv = new ParameterInt(sumLinks);
		node->parameters[param] = pv;
	}

	ElementFactory::maxValues[param] = maxSumLinks;
	ElementFactory::resourcesDeficit[param] = sumVirtualLinks / getNetwork()->getLinksParameters();
...
void Snapshot::commit()
{
	...
	for (;first != last; first++) {
		TenantXMLFactory * f = first.value();
		//ASSIGN NODES
		f->readAssignmentNodesData(*network);
	}

	for (first=tenants.begin(); first != last; first++) {
		TenantXMLFactory * f = first.value();
		//ASSIGN LINKS
		f->readAssignmentLinksData(*network);
	}
}
...
void Snapshot::readVmVmAffinity(const QDomElement & vm_vm_affinity, int & sl) {
	...
	for ( int i = 0; i < rules.size(); i++ ) {
		...
		Requests requests;
		for (Elements::iterator j = vms.begin(); j != vms.end(); j++)
			requests.insert((*j)->request);
...
void Snapshot::printRulesStat() {
	//
	TenantsElements tenants = getTenants();
	//
	...
	for (Elements::iterator i = vmVmAffinity.begin(); i != vmVmAffinity.end(); i++ ) {
		...
		if (unassign) {
			...
			for (Elements::iterator k = elems.begin(); k != elems.end(); k++)
				if ((*k)->isAssigned()) {
					printf("[ERROR] Error in rules assignment");
					break;
				}
...
std::string Snapshot::getPathInHuaweiStyle(Path& route, const ResourcesXMLFactory & resourceFactory) const {
	...
	if (path.front()->toEdge()->getFirst()->getParentNode()->isComputational()) {
		...
		prevNode = path.front()->toEdge()->getSecond()->getParentNode();
	} else {
		port1 = path.front()->toEdge()->getSecond();
		port2 = path.front()->toEdge()->getFirst();
		prevNode = path.front()->toEdge()->getFirst()->getParentNode();
	}
