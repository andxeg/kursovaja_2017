...
Link * Factory::createLink(const QDomElement & e, const ElementsMap& elementsMap) {
    ...
    //channel_capacity
    if ( e.hasAttribute("capacity") ) {
	    uint capacity = e.attribute("capacity").toUInt();
	    if ( capacity == 0 ) {
            qDebug() << "Link capacity is 0 or not Uint";
    ...
    if ( ( elem1->isComputer() || elem1->isStore() )
         && ( elem2->isComputer() || elem2->isStore() ) )
    {
...
Element * Factory::createNode(const QDomElement & e) {
    ...
    if ( type == "server" )
        setNumaBlocks(node->toComputer(), e);
...
void Factory::setNumaBlocks(Computer * node, const QDomElement & e) {
    if (!e.hasAttribute("numa"))
        return;
    ...
    node->toComputer()->numa = numa;
    Parameters parameters = node->getParameters();

    for (size_t j = 0; j < numa; j++) {
        Parameters numaParam;
        for (Parameters::iterator i = parameters.begin(); i != parameters.end(); i++) {
            Parameter *param = i->first;
            ParameterValue *paramValue = i->second;
            if (param->getName() == std::string("RootDisk"))
                continue;

            numaParam[param] = new ParameterInt((int)floor(paramValue->weight() / numa));
        }

        node->numaBlocks.insert(new NumaBlock(numaParam));
    }
}
