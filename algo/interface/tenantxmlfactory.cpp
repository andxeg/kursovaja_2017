...
void TenantXMLFactory::readAssignmentNodesData(const class ResourcesXMLFactory& resourceFactory) {
    //ASSIGN NODES
...
void TenantXMLFactory::readAssignmentLinksData(const class ResourcesXMLFactory& resourceFactory) {
    //ASSIGN LINKS
    QDomNodeList lists_of_links = tenant.elementsByTagName("list_of_links");
    QDomElement list_of_links = lists_of_links.at(0).toElement();
    QDomNodeList virtualLinks = list_of_links.elementsByTagName("link");

    for (int i = 0; i < virtualLinks.size(); i++) {
        QDomElement link = virtualLinks.at(i).toElement();

        if (!link.hasAttribute("assignedTo"))
            continue;

        std::string path = link.attribute("assignedTo").toUtf8().constData();

        Element * virtualNode1 = getElement(link.attribute("node1"));
        Element * virtualNode2 = getElement(link.attribute("node2"));

        Element * virtualLink = NULL;
        Elements edges = virtualNode1->adjacentEdges();
        for (Elements::iterator e = edges.begin(); e != edges.end(); e++) {
            if ((*e)->toEdge()->connects(virtualNode2)) {
                virtualLink = *e;
                break;
            }
        }

        //If TWO VMS ASSINED IN ONE PMS
        if (path.empty()) {
            Path p = Path(virtualNode1->getAssignee(),
                          virtualNode2->getAssignee());
            virtualLink->toLink()->setRoute(p);
            virtualLink->toLink()->setAssignedFlag(true);
            continue;
        }
        ...
        for (size_t k = 0; k < (route.size() - 1); k+=2) {
            std::string node1Name;
            std::string node2Name;
            for (size_t l = 0; l < route[k].size(); l++) {
                char c = route[k][l];
                if (c == ':')
                    break;
                node1Name += c;
            ...
            for (size_t l = 0; l < route[k+1].size(); l++) {
                char c = route[k+1][l];
                if (c == ':')
                    break;
                node2Name += c;
            ...
            Element * node1 = resourceFactory.getElement(QString::fromStdString(node1Name));
            Element * node2 = resourceFactory.getElement(QString::fromStdString(node2Name));
            Elements adjacentEdges = node1->adjacentEdges();
            Element * physicalLink = NULL;
            for (Elements::iterator j = adjacentEdges.begin(); j != adjacentEdges.end(); j++) {
                if ((*j)->toEdge()->connects(node2)) {
                    physicalLink = *j;
                    break;
                }
            }
            ...
            if (physicalLink != NULL) {
                physicalLink->toLink()->assignLink(virtualLink);
                p.addElement(physicalLink);
                continue;
            }
        }
        virtualLink->toLink()->setRoute(p);
...
void TenantXMLFactory::assignLink(Link * link, const ResourcesXMLFactory & resourceFactory) {
    ...
    if (link->getFirst()->getParentNode() == link->getSecond()->getParentNode()) {
        element.setAttribute("assignedTo", "");
        return;
    }
...
QString TenantXMLFactory::getPathXml(Path& route, const ResourcesXMLFactory & resourceFactory) const {
    ...
    if (path.front()->toEdge()->getFirst()->getParentNode()->isComputational()) {
        port1 = path.front()->toEdge()->getFirst();
        port2 = path.front()->toEdge()->getSecond();
    } else {
        port1 = path.front()->toEdge()->getSecond();
        port2 = path.front()->toEdge()->getFirst();
    }
    ...
    for (std::vector<Element *>::iterator it = ++path.begin(); it != path.end(); ++it) {
        ...
        if ( !e->isEdge() )
            continue;
        port1 = e->toEdge()->getFirst();
        port2 = e->toEdge()->getSecond();
        ...
        if (port1->getParentNode() == prevNode) {
            ...
            unjoinedResult << getPhysicalPortXML(port2, resourceFactory)
            << getPhysicalPortXML(port1, resourceFactory);
...
Element * TenantXMLFactory::getElement(const QString & name) const {
